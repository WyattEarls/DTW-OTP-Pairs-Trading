...
This paper compares Dynamic Time Warping (DTW) and Optimal Causal Path (OCP) as distance metrics for pair selection in pairs trading, applied to a survivorship-bias-free universe of S&P 500 constituents. Each metric forms the basis for Ward-linkage hierarchical clustering, with candidate pairs identified through Engle-Granger cointegration testing within clusters, corrected for multiple comparisons using the Benjamini-Hochberg procedure, and traded out of sample using a Kalman-filter hedge ratio and a regime-dependent entry rule. OCP's causality constraint and DTW's unconstrained alignment produce comparably lopsided cluster structures, suggesting the imbalance reflects the underlying data rather than the constraint itself. Restricting candidate selection to Train-period data reverses which method identifies more candidate pairs and eliminates the implausibly high Sharpe ratios present under the uncorrected specification. Among the pairs that traded out of sample, performance was positive under both metrics, though small active-pair counts limit how confidently any performance gap can be attributed to the distance metric itself.
...
The remainder of this paper proceeds as follows. Section 2 reviews the DTW and OCP literature and situates our comparison within it. Section 3 details the construction of the stock universe, the two distance metrics, and the clustering, cointegration, hedge ratio and trading rule methodology. Section 4 presents the resulting cluster structure and Test-period trading performance. Section 5 reports two robustness checks, one addressing look-ahead bias in candidate pair selection and one addressing trade-count heterogeneity across comparison groups. Section 6 discusses the limitations of the current design and directions for future work.
